...
from transformers import (AdamW, get_linear_schedule_with_warmup,
                          RobertaConfig, BertForMaskedLM, RobertaForMaskedLM, AutoConfig, AutoModel,
                          AutoModelForSeq2SeqLM)
from utils import getargs, set_seed, TextDataset
import warnings

warnings.filterwarnings(action='ignore')
from model_one import Ensemblemodel
...
def test(args, model, tokenizer):
    ...
    result_all = pd.DataFrame(result, index=[0])
    result_all.to_csv(os.path.join(args.result_dir, args.pro + "_test_Metrics.csv"), index=False)
    ...
    if not os.path.exists(args.result_dir):
        os.makedirs(args.result_dir)
    dfScores.to_csv(os.path.join(args.result_dir, args.pro + "_val_Metrics.csv"), index=False)
...
def get_basic_model(model, config=None):
    ...
    nl_model = model_class.from_pretrained(model,
                                           config=config, from_tf=False, ignore_mismatched_sizes=True)
    tokenizer = tokenizer_class.from_pretrained(model, do_lower_case=True)
    return nl_model, config, tokenizer


def get_basic_model_2(model, config=None):
    config_class, model_class, tokenizer_class = MODEL_CLASSES['roberta']
    if not config:
        config = config_class.from_pretrained(model)
    else:
...
def main():
    ...
    textEncoder = RobertaModel.from_pretrained(args.text_model_path, config=config)

    if args.model == 'model_one':
        model = Ensemblemodel(textEncoder,
                              config.hidden_size,
                              args.num_class)
    ...
    if args.n_gpu > 1:
        ...
        # args.device = torch.device("cuda")  # 指定gpu2为主GPU
